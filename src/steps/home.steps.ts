...
When(/^i click on sign in link$/, async function () {
    ...
    await (await HomePage.signin_val).waitForDisplayed({ timeout: 2000 });
...
When(/^i enter valid email address (.+) in the textbox$/, async function (domain: string) {

    await (makeEmailId(HomePage.emailaddress_val, domain))
});
